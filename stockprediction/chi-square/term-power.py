...
try:
    with connection.cursor() as cursor, connection2.cursor() as cursor2:
        ...
        for row in cursor:
            ...
            #    break
            term = row['term']
            sql3 = "SELECT `polarity`, COUNT(DISTINCT(`title`)) FROM `term-freq` WHERE `seg-tool`=%s AND `alpha`=%s AND \
            `company`=%s AND `term`=%s GROUP BY `polarity`" # term - document frequency            
            cursor2.execute(sql3,(segtool,alpha,company,term))
            for row2 in cursor2:
                if row2['polarity']=='positive':
                    n_pos_freq = row2['COUNT(DISTINCT(`title`))']
                if row2['polarity']=='neutral':
                    n_neu_freq = row2['COUNT(DISTINCT(`title`))']
                if row2['polarity']=='negative':
                    n_neg_freq = row2['COUNT(DISTINCT(`title`))']
            
            sql4 = "SELECT `polarity`, COUNT(DISTINCT(`title`)) FROM `term-freq` WHERE `seg-tool`=%s AND `alpha`=%s AND \
            `company`=%s AND `term`!=%s GROUP BY `polarity`" # term - document frequency            
            ...
            with connection3.cursor() as cursor3:
                try:
                    sql5 = "INSERT INTO `term-power` (`seg-tool`,`alpha`,`company`,`term`,\
